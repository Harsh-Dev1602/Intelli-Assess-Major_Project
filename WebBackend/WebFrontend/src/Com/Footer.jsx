...
function Footer() {
    ...
    return (
        <>
            <div className="w-full rounded-t-xl h-120 lg:h-80 font-bold mt-2 flex flex-col lg:flex-row justify-evenly items-center bg-gray-800 ">
                <ul className='flex flex-col w-[80%] lg:w-auto items-center lg:items-start justify-center p-1 lg:px-15'>
                    {
                        NavText.map(({ id, text, link }) => (
                            <Link to={link} key={id} className='text-white hover:text-indigo-500 hover:underline transition-all duration-500 font-bold hover:bg-[#cccccc1c]  rounded-xl cursor-pointer py-1 px-5'>{text}</Link>
                        ))
                    }
                </ul>
